...
if run_s3D2:
    ...
    cloud_centers_x = [25, 75]
    cloud_centers_y = [25, 25]
    cloud_rs        = [20, 20]

    cloud_coords_x = []
    cloud_coords_y = []
    for cx, cy, r in zip(cloud_centers_x, cloud_centers_y, cloud_rs):
        cloud_coords_x.extend( [x for x in range(cx - r, cx + r) for y in range(cy - r, cy + r)] )
        cloud_coords_y.extend( [y for x in range(cx - r, cx + r) for y in range(cy - r, cy + r)] )
...
if build_r3D_cases:
    ...
    path_MV3D = '/home/gijs-hogeboom/dev/mclw/data_input/'
